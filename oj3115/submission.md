...
```text
input บรรทัดแรกคือ จำนวนของร้านค้าและจำนวนของเวลาที่ต้องการทราบจำนวนร้าน บรรทัดถัดมาคือเวลาเปิดและปิดของรัาน และบรรทัดสุดท้ายคือเวลาทั้งหมดที่ต้องการทราบ output คือจำนวนร้านของระยะเวลานั้น ๆ โดยถ้าเวลา = เวลาเปิดร้านจะถือว่าเปิด แต่ถ้าหาก = เวลาปิด จะไม่ถือว่าเปิดร้าน
```
...
```text
เริ่มต้นจากการรับค่าบรรทัดแรกเข้ามา และสร้างลิสต์​เก็บร้านค้าทั้งหมด โดยแต่ละร้านค้าประกอบด้วยเวลาเปิดปิดของตัวเองเป็นลิสต์​ย่อย ๆ ถัดมา เก็บค่าแต่ละเวลาในลิสต์เวลา แล้วจึงสร้างลูปเพื่อตรวจสอบว่าในเวลานั้น ๆ ร้านทั้งหมดมีร้านใดเปิดบ้าง โดยใช้ลูปซ้อนลูป
```
...
```text
ใช้วิธีเดียวกับวิธีแรก โดยเช็คว่าเวลานั้น ๆ ร้านนั้น ๆ เปิดอยู่หรือไม่ ผ่านการเงื่อนไขว่า เวลาเปิด <= เวลานั้น ๆ < เวลาปิด
```
...
```text
เพราะเป็นเทสเคสที่โจทย์​กำหนด​มา​ให้​และมีเวลาที่ไม่มีร้านใดเปิดเลย
```
...
```text
เพราะเป็นเทสเคสที่โจทย์​กำหนด​มา​ให้​และทุกร้านเปิดในทุกเวลาที่กำหนด
```
...
```text
เพราะไม่มีร้านใดเปิดในเวลาที่กำหนดเลย
```

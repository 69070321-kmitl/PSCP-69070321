...
```text
According to the task requirements, there are 3 inputs: the temperature value​, the original temperature​ unit, and the target temperature​ unit​. And the output is the converted​ temperature​ value.
```
...
```text
First, I designed every condition without using any functions. However, I later realized that this approach made the code unnecessarily long, so I found a simpler way to reduce the code for conversions between two non-Celsius temperature units conditions.
```
...
```text
First, I created a function to convert between Celsius and other temperature units. Then, I created three variables to receive the inputs. After that, I checked whether either the original or the target temperature unit was Celsius. If so, I simply passed the inputs to the function for the conversion. Otherwise, I first converted the original temperature to Celsius, and then converted the Celsius value to the target temperature unit.
```
...
```text
Because it was in the task's test cases and the original temperature ​unit​ was Celsius​.
```
...
```text
Because it was in the task's test cases and the target temperature​ unit​ was Celsius​.
```
...
```text
Because it was in the task's test cases and neither of the two temperature​ unit​s was Celsius.
```

...
```text
input คือลำดับของช่องนั้น ๆ ในรูปสามเหลี่ยม และ output คือจำนวนกำแพงที่จะต้องผ่านจนกว่าจะไปถึงช่องที่ 1
```
...
```text
เริ่มจากการหาแพทเทิร์นจากเทสเคสที่โจทย์ให้มา ซึ่งได้ข้อสรุปว่า จำนวนกำแพงที่จะต้องผ่าน = 2 * n โดยที่ n มาจากรากที่สองของตัวที่อยู่ฝั่งขวาสุดของแถวนั้นๆ ซึ่งมีแพทเทิร์นเป็น 1 4 9 16 ... แล้วลบด้วยด้วย 1 แต่มีเงื่อนไขเพิ่มเติมคือ ไม่ว่าเลขตัวที่อยู่ฝั่งขวาสุดของแถวนั้น ๆ จะเป็นคู่หรือคี่ ถ้าหากเลขในช่องที่กำหนดไม่ใช่เลขชนิดเดียวกัน จำนวนกำแพงจะ - 1
```
...
```text
ใช้วิธีการเหมือนกับแผนแรก โดยมีการ import math เข้ามาใช้ในการหารากที่สอง หากหารากที่สองไม่พอดี จะใช้ตัวถัดไป เช่น 2.828 ทำการปัดเศษแล้ว + 1 จะได้เท่ากับ 3
```
...
```text
เพราะเป็นเทสเคสที่โจทย์กำหนดมาให้
```
...
```text
เพราะ 1 ไม่จำเป็นต้องผ่านกำแพงใด ๆ
```
...
```text
เพราะควรจะมีค่าเท่ากับเทสเคสแรก เนื่องจากอยู่ในแถวเดียวกันและเป็นเลขคี่เหมือนกัน
```

...
```text
input บรรทัดแรกคือพื้นที่ที่น้ำหมึกไหล ตารางเมตร / วินาที  และจำนวนของพิกัดบ้าน 
output คือ เวลาที่แต่ละบ้านจะถูกน้ำท่วมจนหมด
```
...
```text
ณ ตอนแรก ใช้วิธีการวนลูปซ้ำ โดยเริ่มต้นจากการหาค่าการกระจัดระหว่าง x , y และพิกัด 0, 0 และนำมาเปรียบเทียบกับค่ารัศมีซึ่งคำนวนได้จากการกลับข้างสมการ พื้นที่ = pi * (รัศมี) ** 2 โดยค่อย ๆ คำนวนค่ารัศมีไปใยแต่ละวินาที
```
...
```text
เวลาที่โค้ดใช้คำนวนนั้นนานเกิน จึงเลือกใช้วิธีการคูณแทนการค่อย ๆ บวกทีละลูป โดยใช้ พื้นที่ * เวลา = pi * (รัศมี) ** 2 แล้วกลับข้างสมการ หลังจากนั้นจึงจะตรวจสอบว่าผลลัพธ์ที่ได้มีเศษหรือไม่ หากมี เวลาเพิ่มขึ้น 1
```
...
```text
เพราะเป็นเทสเคสที่โจทย์กำหนดมาให้และสามารถเช็คค่าได้หลายรูปแบบในครั้งเดียว
```
...
```text
เพราะเป็นการเช็คว่าหากจุดสิ้นสุดของรัศมีน้ำหมึกเท่ากับพิกัดบ้านจะถือเป็นน้ำท่วมหรือไม่
```
...
```text
เพราะเป็นเลขที่ค่อนข้างใหญ่และหลายตัวทำให้อาจจะต้องใช้เวลามากกว่าเดิม
```
